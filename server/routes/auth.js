...
router.post('/admin/login', async (req, res) => {
  ...
  // 事务：管理员登录时确保 students 表有对应记录（自愈），避免半写
  let user;
  db.transaction(() => {
    user = db.get('SELECT * FROM students WHERE username = ?', [admin.username]);
    if (!user) {
      const result = db.run(`
...
router.post('/student/register', (req, res) => {
  ...
  const result = db.transaction(() => {
    ...
    if (adminCount.length === 0) {
      ...
      return { id: r.lastInsertRowid, role: 'admin' };
    }
    return { id: r.lastInsertRowid, role: 'student' };
  });
